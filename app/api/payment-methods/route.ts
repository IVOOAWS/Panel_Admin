...
export async function GET(request: NextRequest) {
  try {
    ...
    // const response = await fetch(`${process.env.IVOOAPP_API_URL}/payment-methods`, {
    //   headers: {
    ...
    return NextResponse.json(
      { success: false, error: 'Error fetching payment methods' },
      { status: 500 }
    );
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const updatedMethod = togglePaymentMethod(id, isEnabled);
    ...
    //   `${process.env.IVOOAPP_API_URL}/payment-methods/${id}`,
    //   {
    //     method: 'PATCH',
    //     headers: {
    ...
    return NextResponse.json(
      { success: false, error: 'Error updating payment method' },
      { status: 500 }
    );
  }
}
