...
export default function PickupPage() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      // Fetch all orders and filter for pickup-related ones
      ...
      console.log('[v0] Pickup orders fetched:', data);
      
      if (data.success && Array.isArray(data.data)) {
        // In a real scenario, you might have a specific company filter for pickup
        setOrders(data.data);
      }
    } catch (error) {
      console.error('[v0] Error fetching pickup orders:', error);
      toast.error('Error al cargar las órdenes de pickup');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRefresh = async () => {
    ...
    await fetchOrders();
    setRefreshing(false);
    toast.success('Órdenes de pickup actualizadas');
  };
    ...
    <div className="min-h-screen bg-background dark:bg-slate-950 p-4 md:p-8">
      ...
      <div className="mb-8 pb-6 border-b border-gray-200 dark:border-slate-700">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-black dark:text-white mb-2">Gestión de Pickup</h1>
            <p className="text-gray-600 dark:text-slate-400">
              Control de recogidas en tienda desde IVOOAPP
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
          <div className="text-sm text-gray-500 font-semibold uppercase">En Tienda</div>
          <div className="text-3xl font-bold text-green-600 mt-2">
            {orders.filter(o => o.deliveryStatus === 'in_store' || o.deliveryStatus === 'in_progress').length}
          </div>
        </div>
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
