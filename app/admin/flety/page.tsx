...
export default function FletyPage() {
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/orders?company=Flety');
      ...
      console.log('[v0] Flety orders fetched:', data);
      
      if (data.success && Array.isArray(data.data)) {
        setOrders(data.data);
      }
    } catch (error) {
      console.error('[v0] Error fetching Flety orders:', error);
      toast.error('Error al cargar las órdenes de Flety');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleRefresh = async () => {
    ...
    await fetchOrders();
    setRefreshing(false);
    toast.success('Órdenes de Flety actualizadas');
  };
    ...
    <div className="min-h-screen bg-background dark:bg-slate-950 p-4 md:p-8">
      ...
      <div className="mb-8 pb-6 border-b border-gray-200 dark:border-slate-700">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl md:text-4xl font-bold text-black dark:text-white mb-2">Gestión Flety</h1>
